...
{
    public TeachersController(IRepositoryManager repository, ILogger<TeachersController> logger, IMapper mapper) : base(repository, logger, mapper)
    {
    }
    ...
    public async Task<IActionResult> GetTeacher(int teacherId)
    {
        ...
        if (teacher is null)
        {
            _logger.LogInformation($"Teacher with id: {teacherId} doesn't exist in the database.");
            return NotFound();
        }
        else
        {
